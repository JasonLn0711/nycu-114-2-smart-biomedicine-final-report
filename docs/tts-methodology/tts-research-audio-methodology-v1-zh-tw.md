...
-> automated QA (`tools/run_tts_auto_qa.py`)
-> local repair
-> full render
-> loudness/stitching
-> final package
...
final video manifest、submission evidence checklist、AI/TTS disclosure 屬於交付層，
不放進 TTS 方法論主判斷。交付層文件放在 `docs/delivery/`，用來記錄影片版本、
YouTube / Google Drive / Padlet 狀態、平台 smoke check、字幕與揭露文字。
